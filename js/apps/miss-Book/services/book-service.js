...
const BOOK_KEY = "book";
const BOOK_API = "bookApi";

const BOOKS_CACHE = "booksCache";
...
function getGoogleBooksApi(searchStr) {
  console.log("getting cache");
  console.log("getting axios");
  return axios
    .get(
      `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchStr}`
    )
    .then((res) => {
...
function query() {
  return storageService.query(BOOK_KEY).then((books) => {
    if (!books || !books.length) {
      storageService.postMany(BOOK_KEY, gBooks);
      return gBooks;
    }
    return books;
  });
...
function _createBooks() {
  ...
  if (!books || !books.length) {
    books = [];
    books.push(_createCar("", 300));
    books.push(_createCar("", 120));
    books.push(_createCar("", 100));
    books.push(_createCar("", 150));
